  ...
  return (
    <>
        ...
        {assignments.map((assignment) => (
          <div key={assignment.id}>
            <Row className="display-assignments">
              <Col span={3} className="assignment-name">
                {assignment.id}
              </Col>
              ...
              <Col span={1}>
                <Tooltip
                  title={
                    <>
                      <span className="tooltip-label">Course Name:</span>{" "}
                      <span className="tooltip-value">{`${assignment.courseName}`}</span>
                      <br />
                      <span className="tooltip-label">Date:</span>{" "}
                      <span className="tooltip-value">{`${assignment.date}`}</span>
                      <br />
                      <span className="tooltip-label">
                        Submission Type:
                      </span>{" "}
                      <span className="tooltip-value">{`${assignment.submissionType}`}</span>
                      <br />
                      <span className="tooltip-label">
                        Number of Files:
                      </span>{" "}
                      <span className="tooltip-value">{`${assignment.numberOfFiles}`}</span>
                      <br />
                      <span className="tooltip-label">File Type:</span>{" "}
                      <span className="tooltip-value">{`${assignment.fileType}`}</span>
                    </>
                  }
                >
                  <InfoCircleOutlined className="view-asm-detail-icon" />{" "}
                  {/* Hover to show the assignmemt detai for marking */}
                </Tooltip>
              </Col>
              <Col span={2}>
                {assignment.privacyProtectionState === "start" ? (
                  <FaGooglePlay
                    className="start-eva-icon"
                    size={18}
                    onClick={() => modalAskInputUser(assignment.id)}
                  />
                ) : (
              ...
              <Col span={8}>
                <ProgressBar
                  now={assignment.progressPercentage}
                  label={`${assignment.progressPercentage}%`}
                  visuallyHidden
                />
                {assignment.privacyProtectionState === "process" && (
                  <p className="progress-announcement">
                    The system is removing the sensitive data from the student
                    works.
                  </p>
                )}
              </Col>
      ...
      <Modal
        ...
        open={isModalOpen}
        onCancel={cancelSensiData}
        onOk={() => confirmSensiData(currentAssignment)}
        width={650}
        okButtonProps={{
      ...
      >
        ...
        <p className="step-description">
          Please choose the type of sensitive data that you want to hide
        </p>
        <div className="sensi-data-choose">
            ...
            {options.map((option, index) => (
              <div key={index}>
                ...
                <CloseOutlined
                  className={
                    options.length > 3
                      ? "delete-sensi-data-option"
                      : "del-sensi-data-option-disable"
                  }
          ...
          <Input
            className="input-new-op"
            value={newOption}
            onChange={inputNewOpt}
          />
          <Button
            className="add-sensidata-button"
            onClick={allowAddNewOption}
            disabled={newOption.length < 3}
          >
            Add
          </Button>
        </div>
